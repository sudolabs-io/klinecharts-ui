...
export default class DefaultDatafeed implements Datafeed {
  ...
  async getSymbolDetails(symbol: string): Promise<SymbolDetails> {
    const response = await fetch(`https://api.polygon.io/v3/reference/tickers/${symbol}?apiKey=${this._apiKey}`)
    const result = await response.json()
    
    return {
      ticker: result.results.ticker,
      name: result.results.name,
      shortName: result.results.ticker,
      market: result.results.market,
      exchange: result.results.primary_exchange,
      priceCurrency: result.results.currency_name,
      type: result.results.type,
      logo_url: result.results.branding?.logo_url,
