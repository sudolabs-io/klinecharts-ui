...
export default class DefaultDatafeed implements Datafeed {
  ...
  async getSymbolDetails(symbol: SymbolInfo): Promise<SymbolDetails> {
    const response = await fetch(`https://api.polygon.io/v3/reference/tickers?apiKey=${this._apiKey}&active=true&ticker=${symbol.ticker}`)
    const result = await response.json()
    return await (result.results || []).map((data: any) => ({
      ticker: data.ticker,
      name: data.name,
